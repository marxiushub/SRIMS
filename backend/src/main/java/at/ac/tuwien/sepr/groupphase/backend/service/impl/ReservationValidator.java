...
public class ReservationValidator {
    ...
    public void isEquiomentAvailable(Equipment equipment, LocalDate start, LocalDate end) throws IllegalArgumentException, ValidationException{
        ...
        for (TimePeriods time: timePeriodsList){
            if (time.getStartDate().isBefore(end) && time.getEndDate().isAfter(start) ){
                if (time.getPeriodType() == PeriodType.RENTED) throw new ValidationException("Equipment already reserved in this time range");
                else throw  new ValidationException("Equipment not available at this Date");
            }
        }
